...
{	
	...
	template <int N, class T, class DT, bool CopyData = false, bool Continuous = true, class ArrayRefCountT = EmptyClass, class GridRefCountT = EmptyClass>
	...
	public:
		...
		typedef boost::const_multi_array_ref<DT, N> array_type;
		typedef std::unique_ptr<array_type> array_type_ptr;

		array_type_ptr m_pF;
		ArrayRefCountT m_ref_F;					// reference count for m_pF
		std::vector<DT> m_F_copy;						// if CopyData == true, this holds the copy of F
	...
	/*	template <class IterT1, class IterT2, class IterT3, class RefCountIterT>
		...
		void set_grids(IterT1 grids_begin, IterT2 grids_len_begin, bool bCopy) {
			...
			for (int i = 0; i<N; i++) {
				...
				if (bCopy == false) {
					T const *grid_ptr = &(*grids_begin[i].begin());
					m_grid_list.push_back(grid_type(gridLength, (T*)grid_ptr));	  				// use the given pointer
				}
				else {
					m_grid_copy_list.push_back(std::vector<T>(grids_begin[i].begin(), grids_begin[i].begin() + grids_len_begin[i]));	// make our own copy of the grid
	...
	/*	template <class IterT1, class RefCountIterT>
		...
		void set_f_array(IterT f_begin, IterT f_end, bool bCopy) {
			...
			if (bCopy == false) {
				...
				m_F_copy = std::vector<DT>(f_begin, f_end);
		...
		DT get_f_val(std::array<int, N> const &cell_index, std::array<int, N> const &v_index) const {
		...
		DT get_f_val(std::array<int, N> const &cell_index, int v) const {
	...
	template <int N, class T,class DT, bool CopyData = true, bool Continuous = true, class ArrayRefCountT = EmptyClass, class GridRefCountT = EmptyClass>
	class InterpSimplex : public NDInterpolator<N, T, DT, CopyData, Continuous, ArrayRefCountT, GridRefCountT> {
	public:
		typedef NDInterpolator<N, T, DT, CopyData, Continuous, ArrayRefCountT, GridRefCountT> super;
		...
		void interp_vec(int n, IterT1 coord_iter_begin, IterT1 coord_iter_end, IterT2 i_result) const {
			...
			std::array<int, N> cell_index, v_index;
			std::array<std::pair<T, int>, N> xipair;
			int c;
			T y, v0, v1;
			//mexPrintf("%d\n", n);
	...
	template <int N, class T,class DT, bool CopyData = true, bool Continuous = true, class ArrayRefCountT = EmptyClass, class GridRefCountT = EmptyClass>
	class InterpMultilinear : public NDInterpolator<N, T,DT, CopyData, Continuous, ArrayRefCountT, GridRefCountT> {
	public:
		typedef NDInterpolator<N, T,DT, CopyData, Continuous, ArrayRefCountT, GridRefCountT> super;
		...
		template <class IterT1, class IterT2>
		static DT linterp_nd_unitcube(IterT1 f_begin, IterT1 f_end, IterT2 xi_begin, IterT2 xi_end) {
			int n = xi_end - xi_begin;
			int f_len = f_end - f_begin;
			assert(1 << n == f_len);
			DT sub_lower, sub_upper;
			if (n == 1) {
				sub_lower = f_begin[0];
				sub_upper = f_begin[1];
			}
			...
			DT result = sub_lower + (*xi_begin)*(sub_upper - sub_lower);
			return result;
		}

		template <class IterT>
		DT interp(IterT x_begin) const {
			std::array<DT, 1> result;
			std::array< std::array<T, 1>, N > coord_iter;
			for (int i = 0; i<N; i++) {
				coord_iter[i][0] = x_begin[i];
			}
	...
	typedef InterpSimplex<1, double,double> NDInterpolator_1_S;
	typedef InterpSimplex<2, double, double> NDInterpolator_2_S;
	typedef InterpSimplex<3, double, double> NDInterpolator_3_S;
	typedef InterpSimplex<4, double, double> NDInterpolator_4_S;
	typedef InterpSimplex<5, double, double> NDInterpolator_5_S;
	typedef InterpMultilinear<1, double,double> NDInterpolator_1_ML;
	typedef InterpMultilinear<2, double,double> NDInterpolator_2_ML;
	typedef InterpMultilinear<3, double,double> NDInterpolator_3_ML;
	typedef InterpMultilinear<4, double,double> NDInterpolator_4_ML;
	typedef InterpMultilinear<5, double,double> NDInterpolator_5_ML;
	...
	void linterp_simplex_1(double **grids_begin, int *grid_len_begin, double *pF, int xi_len, double **xi_begin, double *pResult) {
		...
		InterpSimplex<N, double,double, false> interp_obj(grids_begin, grid_len_begin, pF, pF + total_size);
